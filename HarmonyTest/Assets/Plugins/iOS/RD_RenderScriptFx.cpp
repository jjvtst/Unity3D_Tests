...
namespace
{
  ...
  SpriteMapping *createSpriteMapping( const RD_Composition::CompositionNode *node, const RD_SpriteSheet::SpriteData *spriteData, int discretizationStep )
  {
    ...
    if ( isDeformed )
    {
      //  No rotation in sprite matrix .. just transform two bounding coordinates.
      Math::Point3d v1 = spriteData->_matrix * Math::Point3d(spriteData->_rect._x, spriteData->_rect._y, 0);
      Math::Point3d v2 = spriteData->_matrix * Math::Point3d(spriteData->_rect._x+spriteData->_rect._w, spriteData->_rect._y+spriteData->_rect._h, 0);

      xSteps = std::max( 1u, unsigned(fabs(v2.x()-v1.x()) / discretizationStep) );
      ySteps = std::max( 1u, unsigned(fabs(v2.y()-v1.y()) / discretizationStep) );
    }
